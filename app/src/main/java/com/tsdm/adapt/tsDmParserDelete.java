...
public class tsDmParserDelete extends tsDmHandlecmd implements tsDefineWbxml
{
	public int				cmdid;
	...
	public int dmParseDelete(tsDmParser p)
	{
		...
		res = p.dmParseCheckElement(WBXML_TAG_Delete);
		if (res != DM_ERR_OK)
		{
			return res;
		}
		...
		while (true)
		{
			...
			switch (id)
			{
				...
				case WBXML_TAG_Archive:
					is_archive = p.dmParseBlankElement(id);
					break;

				case WBXML_TAG_SftDel:
