...
public class dmFotaEntity implements tsDefineDB {
	...
	public static int startSession() {
		...
		if (!tsService.isNetworkConnect()) {
			tsLib.debugPrintException(DmDevInfoConst.DEBUG_EXCEPTION, "Network disable");
			return -1;
		}

		if ((dmAgent.dmAgentGetSyncMode() > DmDevInfoConst.DM_SYNC_NONE) || (netHttpAdapter.getIsConnected() == true)) {
			tsLib.debugPrintException(DmDevInfoConst.DEBUG_EXCEPTION, "Already Connecting");
			return -1;
		}
		if (dmCommonEntity.startSession()) {
			tsLib.debugPrint(DmDevInfoConst.DEBUG_DM, "");
			dlAgent.dlAgentSetClientInitFlag(DmDevInfoConst.DM_USER_INIT);
	...
	public static void downloadFileFail() {
		tsLib.debugPrint(DmDevInfoConst.DEBUG_UM, "");
		tsDmMsg.taskSendMessage(DmTaskMsg.TASK_MSG_DL_DOWNLOAD_FILE_ERROR, null, null);
		dmAgent.dmAgentSetUserInitiatedStatus(false);
	}

	public static void updateStandby() {
		tsLib.debugPrint(DmDevInfoConst.DEBUG_UM, "");
		tsDmMsg.taskSendMessage(DmTaskMsg.TASK_MSG_DL_FIRMWARE_UPDATE_STANDBY, null, null);
		dmAgent.dmAgentSetUserInitiatedStatus(false);
	}

	public static void updateStart() {
		tsLib.debugPrint(DmDevInfoConst.DEBUG_UM, "updateType= "+tsService.updateType);
		tsDmMsg.taskSendMessage(DmTaskMsg.TASK_MSG_DL_FIRMWARE_UPDATE_START, null, null);
		dmAgent.dmAgentSetUserInitiatedStatus(false);
	}

	public static void updateSuccess() {
		tsLib.debugPrint(DmDevInfoConst.DEBUG_UM, "updateType= "+tsService.updateType);
		tsDmMsg.taskSendMessage(DmTaskMsg.TASK_MSG_DL_FIRMWARE_UPDATE_SUCCESS, null, null);
		dmAgent.dmAgentSetUserInitiatedStatus(false);
	}

	public static void updatePartition() {
		tsLib.debugPrint(DmDevInfoConst.DEBUG_UM, "updateType= "+tsService.updateType);
		tsDmMsg.taskSendMessage(DmTaskMsg.TASK_MSG_DL_FIRMWARE_UPDATE_PARTITION, null, null);
		dmAgent.dmAgentSetUserInitiatedStatus(false);
	}

	public static void updateFail() {
		tsLib.debugPrint(DmDevInfoConst.DEBUG_UM, "updateType= "+tsService.updateType+" uploadFailCause= "+tsService.uploadFailCause);
		tsDmMsg.taskSendMessage(DmTaskMsg.TASK_MSG_DL_FIRMWARE_UPDATE_FAIL, null, null);
		dmAgent.dmAgentSetUserInitiatedStatus(false);
	}

	public static void updateUserCancel() {
		tsLib.debugPrint(DmDevInfoConst.DEBUG_UM, "updateType= "+tsService.updateType);
		tsDmMsg.taskSendMessage(DmTaskMsg.TASK_MSG_DL_FIRMWARE_UPDATE_USERCANCEL, null, null);
		dmAgent.dmAgentSetUserInitiatedStatus(false);
	}
	...
	/*	int nFileId;
		...
		tsLib.debugPrint(DmDevInfoConst.DEBUG_DM, "Percentate [" + nPercentage + "%]");
		...
		if (nDownloadSize > 0 && mDeltaPackageTotalSize > 0) {
			...
			tsLib.debugPrint(DmDevInfoConst.DEBUG_DM, "szDownloadRecvSize [" + szDownloadRecvSize + "]");
	...
	private static void setDownloadTime(long nDownloadSize) {
		...
		if (nDownloadCount == DOWNLOADTIME_CHECK_START_COUNT) {
			...
			tsLib.debugPrint(DmDevInfoConst.DEBUG_DM, "nDownRemainData [" + nDownRemainData + "]");
		} else if (nDownloadCount > DOWNLOADTIME_CHECK_RESET_COUNT) {
			nDownloadCount = 0;
		}

		tsLib.debugPrint(DmDevInfoConst.DEBUG_DM, "DownloadTime [" + szDownloadTime + "]");
	}
}
